 ...
 stages {
  ...
  stage('Prepare') {
   ...
   steps {
    ...
    sh "rm -vf InsightCAEInstaller*"
    sh "rm -vf ${builddir}/insightcae*.tar.gz"
    sh "rm -vf insightcae-wsl-*.tar.xz"
  ...
  stage('Linux-Build') {
   ...
   steps {
    sh './run.sh build'
    sh './run.sh TGZ'
      archiveArtifacts artifacts: "insight-build/*.tar.gz", fingerprint: true
    sh './run.sh package'
      archiveArtifacts artifacts: "insight-build/*.deb, insight-build/*.rpm", fingerprint: true
    }
  }
