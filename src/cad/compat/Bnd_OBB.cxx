...
#include <compat/Bnd_Tools.hxx>
#include <compat/Bnd_Range.hxx>

#include <compat/BVH_BoxSet.hxx>
#include <BVH_LinearBuilder.hxx>

#include <compat/BVH_Traverse.hxx>
...
private:
                ...
                gp_XYZ* thePntMin = 0, gp_XYZ* thePntMax = 0)
  {
    ...
    if (myOptimal)
    {
      ...
      anExtremePointsSelector.SetBVHSet ( &(*myPointBoxSet) );
...
private:
  ...
  NCollection_Handle<BVH_BoxSet <Standard_Real, 3, gp_XYZ>> myPointBoxSet;
                                                   ...
                                                   myOptimal (theIsOptimal)
{
  if (myOptimal)
  {
    ...
    NCollection_Handle<BVH_Builder<Standard_Real, 3> > aLBuilder =
      new BVH_LinearBuilder<Standard_Real, 3> (30);
    myPointBoxSet = new BVH_BoxSet <Standard_Real, 3, gp_XYZ>;
    myPointBoxSet->SetBuilder(aLBuilder);
...
void OBBTool::BuildBox(Bnd_OBB& theBox)
{
  ...
  gp_XYZ aFCurrPoint = myPntsList.Value(myPntsList.Lower()).XYZ();
  ...
  if(myListOfTolers != 0)
  {
    const Standard_Real aTol = myListOfTolers->Value(myListOfTolers->Lower());
    aParams[0] -= aTol;
    aParams[1] += aTol;
    aParams[2] -= aTol;
    aParams[3] += aTol;
                      ...
                      const Standard_Boolean theIsOptimal)
{
  ...
  {
    case 1:
      ProcessOnePoint(theListOfPoints.Value(theListOfPoints.Lower()));
      if(theListOfTolerances)
        Enlarge(theListOfTolerances->Value(theListOfTolerances->Upper()));
      return;
    case 2:
    {
      const Standard_Real aTol1 = (theListOfTolerances == 0) ? 0.0 :
                                      theListOfTolerances->Value(theListOfTolerances->Lower());

      const Standard_Real aTol2 = (theListOfTolerances == 0) ? 0.0 :
                                      theListOfTolerances->Value(theListOfTolerances->Upper());
      
      const gp_XYZ &aP1 = theListOfPoints.Value(theListOfPoints.Lower()).XYZ(),
                   &aP2 = theListOfPoints.Value(theListOfPoints.Upper()).XYZ();
