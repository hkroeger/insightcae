...
setFieldsConfiguration::setFieldsConfiguration( OpenFOAMCase& c, ParameterSetInput ip )
...
void setFieldsConfiguration::addIntoDictionaries(OFdicts& dictionaries) const
{
    ...
    for (const Parameters::defaultValues_default_type& dfv: p().defaultValues)
    {
      if (const auto * sdfv = boost::get<Parameters::defaultValues_default_scalar_type>(&dfv))
        {
          dfvs.push_back(
              boost::str(boost::format("volScalarFieldValue %s %g\n")
    ...
    OFDictData::list rs;
    for (const Parameters::regionSelectors_default_type& r: p().regionSelectors)
    {
      if (const auto * box = boost::get<Parameters::regionSelectors_default_box_type>(&r))
        {
          OFDictData::list vl;
          for (const Parameters::regionSelectors_default_box_type::regionValues_default_type& bv:
               box->regionValues)
          {
            if (const auto * s =
                  boost::get<Parameters::regionSelectors_default_box_type::regionValues_default_scalar_type>(&bv))
              {
                vl.push_back(
                    boost::str(boost::format("volScalarFieldValue %s %g\n")
                               % s->name % s->value));
                       ...
                       boost::get<Parameters::regionSelectors_default_box_type::regionValues_default_vector_type>(&bv))
              {
                vl.push_back(
                      boost::str(boost::format("volVectorFieldValue %s %s\n")
