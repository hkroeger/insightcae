...
protected:
  ...
  std::time_t file_content_timestamp_;
...
public:
  ...
  FileContainer(const boost::filesystem::path& originalFileName,
                std::shared_ptr<std::string> binary_content = std::shared_ptr<std::string>());
  FileContainer(const boost::filesystem::path& originalFileName,
                const TemporaryFile& tf);
  FileContainer(const char *binaryFileContent, size_t size, const boost::filesystem::path& originalFileName);
