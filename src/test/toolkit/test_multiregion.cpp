...
public:
  MultiregionTestAnalysis()
    : OpenFOAMAnalysis("multiregion test", "", OpenFOAMAnalysis::defaultParameters(), ".")
  {
  }
  ...
  void createCase(OpenFOAMCase& cm, ProgressDisplayer& progress) override
  {
    // fluid case
    {
      auto cm_fluidPtr = std::make_shared<OpenFOAMCase>();
  ...
  //      parameters_.get<SelectableSubsetParameter>("fluid/turbulenceModel"));
    ...
    // solid case
    {
      auto cm_solidPtr = std::make_shared<OpenFOAMCase>();
