...
void calcIncompressibleTwoPhase
(
    const fvMesh& mesh,
    const Time& runTime,
    const volVectorField& U,
    const word& phaseFieldName,
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    argList::validOptions.insert("twoPhase", "alpha1");
    ...
    forAll(timeDirs, timeI)
    {
	    ...
	    /*
            ...
            {
	      word phaseFieldName(args.options()["twoPhase"]);
              calcIncompressibleTwoPhase(mesh, runTime, U, phaseFieldName, wallShearStress);
            }
            else
            {
